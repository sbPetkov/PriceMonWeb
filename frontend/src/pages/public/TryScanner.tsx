...
const TryScanner = () => {
  ...
  // Detect page activity for camera
  useEffect(() => {
    const isActive = window.location.pathname === '/try-scanner';
    setIsPageActive(isActive);

    const handleVisibilityChange = () => {
      setIsPageActive(!document.hidden && window.location.pathname === '/try-scanner');
    };
    ...
    try {
      ...
      if (response.found) {
        if (response.status === 'approved' && response.product) {
          // Product exists and is approved → go to public product view
          navigate(`/try-product/${response.product.id}`);
        ...
        // Product not found → show message with CTA to register
        setError('Product not found. Sign up to add it to our database!');
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex items-center justify-center p-4 py-12">
        <div className="max-w-md w-full">
          {/* CTA Banner */}
          <div className="mb-6 bg-gradient-to-r from-primary to-primary-600 text-white rounded-2xl p-6 shadow-lg">
            <div className="flex items-start gap-3 mb-3">
              <svg className="w-6 h-6 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
              </svg>
              <div>
                <h3 className="font-bold text-lg mb-1">Try PriceMon Demo</h3>
                <p className="text-white/90 text-sm">
                  Search for products and view prices. Sign up to add prices, save favorites, and more!
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-white text-primary font-semibold py-2 px-4 rounded-lg hover:bg-white/90 transition-colors text-sm"
              >
                Sign Up Free
              </button>
              <button
                onClick={() => navigate('/login')}
                className="flex-1 bg-primary-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-primary-800 transition-colors text-sm"
              >
                Log In
              </button>
            </div>
                ...
                <div className="flex items-start gap-3 text-sm text-gray-600">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1 list-disc list-inside">
                      ...
                      <li>Sign up to add new products and prices!</li>
                    </ul>
                  </div>
                </div>
              </div>
